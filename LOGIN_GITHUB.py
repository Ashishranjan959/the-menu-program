...
def CheckLogin():
    ...
    if nameEL.get() == uname and pwordEL.get() == pword: # Checks to see if you entered the correct data.
        ...
        r.geometry('550x500') # Makes the window a certain size



        #program for mail sending
        
        

        def mail():
            self='ashish.ranjan959@gmail.com'
            paswd='7858044520'
            content=msg_text
            try:
                ...
                mail.ehlo()
                mail.starttls()
                mail.login(self,paswd)
                mail.sendmail(self,to_text,msg_text)
                mail.close()
            except Exception:
                print("An Exception occur ........check ur mail setting")


        window=Tk()

        f1=Frame(height=500,width=500)

        window.title("Send E-mail")

        l1=Label(window,text='To')
        l1.grid(row=0,column=0)

        to_text=StringVar()
        e1=Entry(window,textvariable=to_text)
        e1.place(x=0,y=20)


        msg_text=StringVar()
        e2=Entry(window,width=50,textvariable=msg_text)
        e2.place(x=0,y=50)



        b1=Button(window,text="Send Email",width=12,command=mail)
        b1.place(x=30,y=80)
